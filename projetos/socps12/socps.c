...
PROCESS_THREAD(socps_process, ev, data)
{
  ...
  while(true) {
    ...
    if((i % 5)==0){
    	process_post(&socps_process2,PROCESS_EVENT_CONTINUE,NULL);
    }
...
PROCESS_THREAD(socps_process2, ev, data)
{
  ...
  while(true) {
    ...
    if(PROCESS_EVENT_MSG == ev){
    	printf("### OCORREU UM EVENTO 1 - PROCESS_EVENT_MSG ###\n");
    }

    if(PROCESS_EVENT_CONTINUE == ev){
        printf("### OCORREU UM EVENTO 2 - PROCESS_EVENT_CONTINUE ###\n");
    }
